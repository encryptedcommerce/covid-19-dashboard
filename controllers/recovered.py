...
from models import recovered as model
from views import recovered as view
...
@app.callback(Output('tab-heading-recovered', 'children'), [Input('url', 'pathname')])
def localize_heading(dummy_url: str = None) -> str:
    return localize('recovered')


@app.callback(Output('tabs-content-recovered', 'children'),
              [Input('tabs-recovered', 'value')])
def render_tab_content(tab):
    return common.render_tab_content(view, tab)


@app.callback(
    Output('recovered-table-component', 'columns'),
    [
        Input('url', 'pathname'),
    ]
)
...
@app.callback(
    Output('recovered-table-component', 'data'),
    [
        Input('url', 'pathname'),
    ]
)
...
@app.callback(
    Output('recovered-download-link', 'href'),
    [
        Input('url', 'pathname'),
    ]
)
...
@app.callback(
    Output('recovered-chart-component', 'figure'),
    [
        Input('recovered-country-selector-dropdown', 'value'),
        Input('url', 'pathname'),
    ]
)
...
@app.callback(
    Output('recovered-country-selector-dropdown', 'options'),
    [
        Input('url', 'pathname'),
    ]
)
...
@app.callback(
    Output('recovered-country-selector-dropdown', 'value'),
    [
        Input('recovered-country-selector-dropdown', 'options'),
    ]
)
